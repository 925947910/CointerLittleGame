-module(oauth_uri).
-include("common.hrl").
-export([normalize/1,normalize/3,normalize/5, calate/2, calate2/2,encode/1,encode2/1]).
...
normalize(URI) ->
  case http_uri:parse(URI) of
    {ok,{_Scheme, _UserInfo, _Host, _Port, Path, _Query}} ->
...
params_to_string(Params) ->
  encode(intercalate("&", [calate2("=", [K, V]) || {K, V} <- Params])).
...
decode(Chars) ->http_uri:decode(Chars).
%%   decode(Chars, []).
%% 
%% decode([], Decoded) ->
